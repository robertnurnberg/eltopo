...
MeshMerger::PreMeshMergeInfo::PreMeshMergeInfo(size_t edge_index_a, 
                                               size_t edge_index_b, 
                                               const std::vector<size_t>& triangles_to_delete, 
                                               const std::vector<Vec3st>& triangles_to_add ) :
    m_edge_index_a(edge_index_a),
    m_edge_index_b(edge_index_b),
...
MeshMerger::PostMeshMergeInfo::PostMeshMergeInfo( const PreMeshMergeInfo& pre_merge_info ) :
    m_pre_merge_info(pre_merge_info)
{}
...
bool MeshMerger::zipper_edges( size_t edge_index_a, size_t edge_index_b )
{
    ...
    PreMeshMergeInfo pre_merge_info( edge_index_a, edge_index_b, deleted_triangles, new_triangles );
    
    for ( size_t i = 0; i < m_observers.size(); ++i )
    {
        if ( false == m_observers[i]->operationOK(m_surf, pre_merge_info) )
        {
            return false;
        }
    }
    ...
    PostMeshMergeInfo post_merge_info(pre_merge_info);
    
    for ( size_t i = 0; i < m_observers.size(); ++i )
    {
        m_observers[i]->operationOccurred(m_surf, post_merge_info);
    }
...
// --------------------------------------------------------
///
/// Perform one pass over the mesh, attempting to merge near edges.
///
// --------------------------------------------------------
...
void MeshMerger::add_observer( MeshMergeObserver* observer )
{
